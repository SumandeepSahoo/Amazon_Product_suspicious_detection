...
export default function PredictForm() {
  ...
  const getSignals = (result) => {
    ...
    if (result.price_anomaly_z > 2) {
      signals.push("📉 Price anomaly detected, Price is significantly lower than expected, Large deviation from category average, Possible fake discount or scam pricing");
    }

    if (result.clip_image_text_sim < 0.5) {
      signals.push("🧠 Text similarity mismatch, Image does not match product description, Low semantic similarity between text and image, Possible misleading or incorrect visuals, Product details may not represent actual item");
    }

    if (result.suspicion_score > 0.6) {
      signals.push("⚠️ Seller pattern risk, Behavior matches known risky seller patterns, Unusual listing activity detected, Multiple weak fraud signals combined, Higher probability of suspicious seller behavior");
    }

    if (signals.length === 0) {
      signals.push("✅ No major risk signals detected, Listing appears consistent with expected patterns, No significant anomalies found in price or content, Image and description are well aligned, Overall product listing seems trustworthy");
    }
        ...
        {result && (
          <div style={styles.resultGrid}>
            ...
            <div style={styles.resultCard}>
              ...
              {getSignals(result).map((signal, index) => (
